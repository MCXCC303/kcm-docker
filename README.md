...
> 创建容器、拉取 / 删除 / **构建**镜像、清理构建缓存、创建 / 删除网络、连接 / 断开网络、
> 创建 / 删除 / 清理数据卷，只在 socket 对当前用户**可写**时出现。除此之外，五期引入了一个**能力被严格限制**的提权 helper（只做两件事：
> 改 `/etc/docker/daemon.json`、重启 docker 服务），且仓库凭据只存 **KWallet**、绝不写明文。
> 详见[写操作与权限](#写操作与权限)。
...
- 六期（[ARCH_V5_V8.md](ARCH_V5_V8.md)）：看得见运行中的容器，管得住网络与数据 ——
  容器**日志流式显示**、网络管理（创建 / 删除 / 连接 / 断开）、数据卷管理（创建 / 删除 / 清理未使用）
- 七期（[ARCH_V5_V8.md](ARCH_V5_V8.md)）：把创建搬进界面 —— **创建容器向导**（分步表单 + 只读总览）、
  **挂载预设**（增删改/排序/收藏）、从现有容器**克隆**配置
- 八期（[ARCH_V5_V8.md](ARCH_V5_V8.md)）：**从 Dockerfile 构建镜像** —— 本地打包上下文
  （尊重 `.dockerignore`、越界符号链接跳过）、构建进度与**失败步骤定位**、构建缓存清理
...
| **端口映射** | 芯片 + 连线拓扑：左列容器端口、右列宿主绑定；**同一个容器端口的多个宿主地址合并成一组**（左列只出现一枚芯片，连线从同一个起点分支到每条绑定，分支颜色按容器与绑定确定性分配）；只 `EXPOSE` 未映射的端口单独成组、不画线；连线为纯装饰（`Accessible.ignored`），信息全部由文字承载 |
| **容器日志** | 容器详情「日志」分区的实时控制台：跟随末尾、暂停（继续接收并在恢复时一次补上）、清空、复制全部、断开与重新连接；按 `Config.Tty` 解复用、剥掉 ANSI 颜色、`\r` 进度条按"覆盖当前行"处理；行数/字节双上限（5000 行 / 512 KiB）并提示省略了多少行；**只在分区打开时读取**，离开即断开 |
| **网络管理** | 「网络」标签页（搜索 / 内置与自建过滤 / 排序）与详情（子网、网关、成员容器可跳转、标签与驱动选项）；创建 bridge 网络（实时校验名称/子网/网关）；删除网络（内置网络不出现删除入口）；容器详情可连接/断开网络（支持别名） |
| **数据卷管理** | 「数据卷」标签页（搜索 / 未使用与使用中过滤 / 排序，卡片区分使用中 / 未使用 / **使用情况未知**）与详情（挂载点可复制、可在文件管理器中打开）；创建卷、删除卷（**不提供 force**）、清理未使用卷（**先列出将被删除的卷与可回收空间**再确认）；存储条图例的「数据卷」可跳到该页 |
| **创建容器** | 单页分步向导：镜像 → 基础（名称/网络/重启策略）→ 端口 → 环境变量与标签 → 挂载 → 资源与特权 → **只读总览**；未通过校验不能往下走，总览里**只列环境变量键名**；`--privileged` 需二次确认；支持从现有容器**克隆配置**（不复制运行时状态） |
| **挂载预设** | 存在 `~/.config/kontainerrc`：增删改、排序、收藏；向导里一键添加；容器详情的挂载行可「保存为预设」；创建成功后自动并入"最近使用"（上限 20，淘汰时不动收藏） |
| **构建镜像** | 从 Dockerfile 构建：上下文目录 / 粘贴 Dockerfile 内容 / 多标签 / 目标阶段 / 构建参数 / `--no-cache` / `--pull`；上下文在本地打包（尊重 `.dockerignore`，**越界符号链接跳过**，超限本地报错）；构建列表显示 `第 N/M 步 · 命令` 与进度、可取消，**失败时保留失败步骤与命令**；私有基础镜像自动带上 KWallet 凭据 |
| **系统服务** | 引擎页的「系统服务」卡片：`docker.socket` / `docker.service` / `containerd.service` 的状态与是否开机自启，以及**受限提权**的启动 / 停止 / 重启 / 启用 / 禁用（固定 unit + 固定动词，危险动作二次确认并写明后果）。连接状态会细化成「已连接 / 已连接但有服务未运行 / 未连接」 |
| **输入效率** | 镜像与命令历史用**可搜索下拉**；命令历史包含本地记录（上限 20，重复的提到最前）与已有容器的命令（只作候选、不写盘） |
| **清理构建缓存** | 先显示可回收空间再确认，结果如实报告回收了多少（没有可回收的会明说） |
| **系统运行时配置** | 用户级 / 系统级两个入口：部署形态、配置文件路径与可写性、其余键只读保留；镜像加速器、insecure-registries、并发下载数、日志驱动四项可编辑（「默认」= 删掉该键）；保存前自动备份、可恢复上一版本、保存后可重启 Docker（含"将停止 N 个容器"的后果说明）；需要 root 时先「解锁以编辑」（约 5 分钟），取消或未装 helper 时给出可复制的手动命令 |
| **仓库认证** | 凭据只存 KWallet（专用文件夹），登录先 `POST /auth` 校验成功才写入；支持令牌登录、测试连接、移除；可从 `~/.docker/config.json` 一次性只读导入（逐条勾选、不覆盖已有条目、不执行 credential helper）；私有仓库拉取自动带上凭据，401/403 的失败行与"该仓库还没登录"的提示都能一键跳去登录 |
| 端口拓扑取色 | 连线颜色按"容器 id + 该映射自身字段"确定性取色：同一容器同一条映射刷新、重开、换主题都不变色，同容器内多条映射彼此可区分；颜色为纯装饰，信息仍在两侧芯片文字里 |
...
```bash
# 1) 创建容器：容器页工具栏「创建容器…」
#    期望：镜像步骤只能选本地已有的（没有就勾「本地没有时先拉取」）；名称重复会被挡住并给原因；
#    端口与现有容器冲突会被挡住；末步总览里环境变量**只显示键名**；
#    勾「以扩展特权运行」必须先确认；创建成功后跳到新容器详情页

# 2) 挂载预设：向导的挂载步骤 →「管理预设…」
#    期望：可增删改、收藏、排序；重新打开 KCM 后预设仍在（存在 ~/.config/kontainerrc）
#    提示：临时目录挂载最容易验证 —— 宿主 /tmp/demo、容器 /data

# 3) 克隆：容器详情 footer「克隆…」
#    期望：名称变成 <原名>-copy，命令/入口点/环境/标签/重启策略/端口/挂载都被带过来；
#    创建出来的新容器与旧容器**只有配置相同**，不会带过来运行状态

# 4) 构建镜像：镜像页工具栏「构建镜像…」
#    选一个真实项目目录（含 Dockerfile 与 .dockerignore），填标签 demo:1.0 →「开始构建」
#    期望：列表里出现构建条目并逐行显示「第 N/M 步 · 命令」；构建期间可以继续操作其它标签页；
#    结束后镜像列表出现 demo:1.0；成功后卡片上的「查看镜像」能跳到该镜像详情

# 5) 失败定位：把一个 Dockerfile 的某条 RUN 改成 `RUN exit 1`
#    期望：构建列表里的失败文案**带着失败的步骤与命令**，而不是笼统的"构建失败"

# 6) 清理构建缓存：构建面板底部的「清理构建缓存…」
#    期望：按钮上方的文字先告诉你可回收多少；确认框写明"下次构建要重做被缓存的工作、
#    已构建的镜像不受影响"；清理后存储条里的「构建缓存」段明显变小
```

### 实机验证容器日志与网络/数据卷

```bash
# 1) 日志：进入某个运行中容器的「日志」分区
#    期望：跟随末尾滚动；点「暂停」后停止追加但仍在接收（状态条写明），「继续」后一次补上；
#    「清空」只清显示；「复制全部」得到纯文本；停止容器后显示「已结束」并可「重新连接」
#    提示：用 alpine 跑一个不断输出的命令最容易看出来，例如
#         docker run --rm -d --name log-demo alpine sh -c 'i=0; while true; do echo "line $((i++))"; sleep 1; done'

# 2) 网络：数据卷标签页旁边进「网络」→「创建网络…」
#    填名称 + 子网（例如 demo_net / 172.30.0.0/16）→ 期望列表出现该网络；
#    在该网络详情里删除 → 期望二次确认写明"已连接的容器会失去该网络"；
#    内置的 bridge/host/none 不应出现删除入口

# 3) 容器连接网络：容器详情「网络」分区 →「连接到网络…」→ 选一个未连接的网络 + 别名
#    期望：网络成员里出现该容器；容器详情网络分区出现新网络；「断开」要二次确认

# 4) 数据卷：数据卷标签页 →「创建数据卷…」→ 名称 demo_vol
#    期望：列表出现该卷并标注"未使用"；删除要二次确认且文案写明数据会消失；
#    在一个容器里用一下这个卷（或 docker run -v demo_vol:/data）后，该卷应变为"被 1 个容器使用"

# 5) 清理：点「清理未使用…」→ 期望先列出将被删除的卷名与可回收空间，确认后才删除；
#    正在被容器使用的卷**不应**出现在预览里；使用情况未知的卷也不应出现
```
...
> **0.6.0** 起写操作扩展到网络与数据卷（不新增提权能力，但**不可撤销的操作变多**：
> 删除网络、删除/清理数据卷——确认对话框固定写明后果，清理前先列出将被删除的卷，
> 且"使用情况未知"的卷不在清理范围内）；
> **0.7.0** 起包含创建容器（`--privileged` 必须二次确认）；
> **0.8.0** 起包含构建镜像与清理构建缓存——**Dockerfile 里的命令是在 Docker 守护进程侧执行的**，
> 构建不信任的 Dockerfile 等于让它在你的机器上跑任意命令（与 `docker build` 本身一样），
> 因此不做任何后台自动构建；清理构建缓存不可撤销，且必须先看到"可回收多少"。
...
| 拉取镜像 | 镜像标签页、镜像空状态 | 否 | 提交前校验引用；缺 tag 时显式提示会补 `latest`；可随时取消；私有仓库自动带上 KWallet 里的凭据 |
| 创建 / 删除网络 | 网络标签页、网络详情 | 删除时**是** | 只创建 bridge；内置网络（bridge/host/none）不出现删除入口；确认文案写明"已连接的容器会失去该网络" |
| 连接 / 断开网络 | 容器详情「网络」分区 | 断开时**是** | 连接可填别名；断开写明"容器会失去该网络上的地址与别名，连接可能中断" |
| 创建 / 删除 / 清理数据卷 | 数据卷标签页、卷详情 | 删除与清理**是** | 删除不提供 force（被容器使用时由引擎拒绝）；清理先列出将被删除的卷与可回收空间，"使用情况未知"的卷不清理 |
| 创建容器 | 容器页工具栏「创建容器…」、镜像详情、容器详情「克隆…」 | 提交前二次确认（`--privileged` 时） | 分步校验（重名、端口冲突、镜像是否在本地、挂载路径）；`--privileged` 必须二次确认；失败保留已填内容 |
| 构建镜像 | 镜像页工具栏「构建镜像…」 | 否（但**会执行 Dockerfile 里的命令**） | 上下文在本地打包校验（缺 Dockerfile、超限、越界符号链接）；只在你点「开始构建」后提交 |
| 清理构建缓存 | 镜像页构建面板 | **是**（不可撤销） | 先显示可回收空间；确认文案写明"下次构建要重做被缓存的工作，已构建的镜像不受影响" |
...
| `tst_container_create` | 创建容器的表单 → API 映射（快照断言：`?name=` 在 query、随机端口用空串、tmpfs 走 `Tmpfs`、`NanoCpus` 单位）、校验矩阵、挂载预设的持久化/去重/最近使用淘汰、向导分步校验、克隆带出完整配置、总览不含环境变量值 |
| `tst_http_request_body` | 请求体分块上传：跨块大文件的长度与逐字节内容、`Content-Type`、文件缺失、远端提前关闭、上传中途取消 |
| `tst_service_control` | 服务管理的提权边界：白名单恰好三个 unit、15 种 unit×动词组合全部通过、其它一律拒绝（`sshd.service`、`docker.service; reboot`、路径穿越、大小写变体）、动作名与 helper 槽名双向一致 |
| `tst_build_context` | 上下文打包：内容真的进 tar、`.dockerignore` 子集与取反语义（父目录被排除时子文件不会被救回，与 Docker 一致）、**越界符号链接被跳过**（负例验证）、超限失败且不留临时文件 |
| `tst_image_build` | 构建流解析（`Step N/M : CMD`、缓存行、`progressDetail`、`aux.ID`）、控制器打包并传参、进度写入列表、**失败步骤必须保留**（负例验证）、清理构建缓存的结果文案 |
| `tst_log_frame_reader` | 日志字节流解复用：非 TTY 的 8 字节 stdcopy 帧（半帧跨包、一个包多帧都正确）、TTY 原始字节流、ANSI 转义（含被切断的序列）被剥掉、`\r` 按"覆盖当前行"处理而 CRLF 仍是行尾、畸形帧头与超大长度被安全丢弃 |
| `tst_container_log_controller` | 日志控制器：行数/字节双上限裁剪并如实报数、按时间窗口与字节阈值批处理、**暂停期间继续接收且恢复时一次补上**（负例验证）、`\r` 的"临时行"替换语义、结束原因分类（自然结束 / 容器不存在 / 日志驱动不支持）、断开后迟到内容不进控制台 |
| `tst_network_model` | 网络列表模型与过滤：role 暴露、内容未变不重置、搜索覆盖名称/ID/驱动/子网、内置与自建过滤、排序、控制器接线，以及**读失败时保留列表**（负例验证） |
| `tst_volume_model` | 数据卷模型与过滤：`UsageData` 有/无（**未知显示"—"而不是 0**，负例验证）、`Volumes: null` 的载荷、坏条目跳过、未使用与使用中过滤排除"使用情况未知"（负例验证）、按大小/引用数排序、控制器接线与失败保留 |
...
- 传输：请求体**分块上传**（256 KiB 一块，靠 `bytesWritten` 做背压）、上传阶段独立静默超时、
  上传/取消/远端提前关闭都有用例 ✅
- 上下文：`KTar` 打包 + `.dockerignore` 常用子集（父目录排除时子文件不会被 `!` 救回，**与 Docker 一致**）✅、
  **符号链接指向目录之外一律跳过**（负例验证）✅、大小/文件数上限 ✅、失败不留临时文件 ✅
  （KTar 析构会把文件写回来，因此删除要等它析构 ✅）
- 请求：`POST /build` 的 `t`/`dockerfile`/`buildargs`/`labels`/`target`/`platform`/`nocache`/`pull` ✅、
  私有基础镜像带 `X-Registry-Auth`（复用五期凭据）✅
- 进度与失败定位：逐行 JSON 进度、`第 N/M 步 · 命令`、**失败时保留失败的步骤与命令原文**
  （负例验证）✅、可单独取消 ✅、结束后删除临时上下文 ✅
- 缓存清理：先显示可回收空间、确认后清理、如实报告回收字节数（0 也明说）✅
- 界面：内联构建表单（不做模态框）、构建列表与拉取列表同一模式、成功后可「查看镜像」✅、
  亮/暗与中文截图复核 ✅
- 工程：`project VERSION` = 0.8.0 ✅、42 个测试目标全部通过 ✅、`-Wall -Wextra` 0 警告 ✅、
  `po/zh_CN` 完整（704 条、0 fuzzy、0 未翻译）✅、`kcmshell6 --smoke-test` 中英文退出 0 ✅
- ⏳ 需在真实环境验收：真实项目的一次完整构建（含 `.dockerignore` 与私有基础镜像）、
  失败构建的步骤定位、构建缓存清理前后的空间对比

---

## 七期完成定义（DoD）自查

- 表单 → API 的映射是**单一纯函数**并用快照断言钉死（`?name=` 在 query、随机端口用空串、
  tmpfs 走 `Tmpfs`、空字段不写进 JSON）✅
- 校验矩阵：名称规则/重名、镜像是否在本地（可「先拉取」放行）、宿主端口冲突、
  挂载路径与来源格式、内存/CPU 范围——依赖后端数据的检查在 C++ 侧，失败给稳定 key 且不发请求 ✅
- 创建：`POST /containers/create`，**「创建并启动」两步串行**且结果分别呈现（启动失败明说是哪一步）✅、
  201 却没有 `Id` 视为失败 ✅
- 向导：分步且未通过不能往下走、末步只读、`--privileged` 二次确认（负例验证）、
  总览只列环境变量键名 ✅
- 预设：`~/.config/kontainerrc` 持久化、稳定 id、收藏置顶、最近使用上限 20、
  淘汰不动收藏与从未用过的预设 ✅，且**测试不写用户真实配置**（注入临时路径）✅
- 克隆：带出命令/入口点/环境/标签/工作目录/用户/重启策略/端口/挂载（负例验证），
  **只复制配置不复制运行时状态** ✅
- 工程：42 个测试目标全部通过 ✅、0 警告 ✅、翻译完整 ✅、smoke 中英文通过 ✅、
  亮/暗与中文截图复核 ✅
- ⏳ 需在真实环境验收：用向导创建一个真实容器（含端口/挂载/环境变量）、
  从现有容器克隆并改名创建、预设的增删改在下次启动后仍然在

---

## 六期完成定义（DoD）自查

- 容器日志：`GET /containers/{id}/logs` 跟随、等宽控制台、跟随/暂停/清空/复制/重连 ✅、
  按 `Config.Tty` 分形态解复用（TTY 原始字节 / 非 TTY 的 8 字节帧）✅、ANSI 与 `\r` 处理 ✅、
  行数/字节双上限与"已省略 N 行"提示 ✅、**进分区才连接、离开即断开、换容器也断开** ✅、
  容器停止 = 正常结束（给"重新连接"）✅、日志驱动不支持读取时给替代做法 ✅、
  **不做**保存/重定向/搜索/高亮/主题定制（范围由用户明确限定）✅
- 网络：列表与详情（成员容器可跳转）✅、创建 bridge 网络（名称/子网/网关校验，重名拦截）✅、
  删除（内置网络**不出现**删除入口，页面说明原因）✅、容器连接/断开（支持别名，断开写明后果）✅、
  创建/删除走 `OperationController` 的同一结果通道与写权限门 ✅、写后即读 ✅
- 数据卷：列表与详情（挂载点可复制、可在文件管理器中打开）✅、创建 ✅、删除（**不提供 force**）✅、
  清理未使用（**先列出将被删除的卷与可回收空间**）✅、**"使用情况未知"不算未使用**（清理与过滤都排除）✅、
  存储条图例的「数据卷」可跳到卷列表 ✅
- 工程：`project VERSION` = 0.6.0 ✅、38 个测试目标全部通过 ✅、`-Wall -Wextra` 0 警告 ✅、
  `po/zh_CN` 完整（553 条、0 fuzzy、0 未翻译）且模板与源码同步由测试强制 ✅、
  新 QML 全部进 qrc 并被 `tst_qml_load` / `tst_qml_resource` 覆盖 ✅、
  集成用例**只碰 alpine/ubuntu 测试容器**（不触碰用户业务容器）✅
- KDE：`kcmshell6 --smoke-test kcm_docker` 英文/中文退出码 0 ✅、亮/暗与中文离屏截图复核 ✅
- ⏳ 需在真实环境验收：真实容器的日志跟随（长输出 + 停止容器）、创建/删除真实网络与数据卷、
  清理未使用卷（确认预览与删除结果一致）、以及 5G 遗留的提权与私有仓库登录项
...
| 六期 ✅ | 对象管理与容器日志 | 容器**日志流式显示**（跟随/暂停/清空/重连，只做显示）+ 网络管理（基础 bridge 创建、连接/断开、删除保护）+ 数据卷管理（创建/删除/不含 force 的清理）——**已交付 0.6.0**，见上方 DoD 自查 |
| 七期 | 创建与部署 | 容器创建分步向导 + 从现有容器克隆 + 挂载预设（`~/.config/kontainerrc`） |
| 八期 ✅ | 构建镜像 | Dockerfile 构建（上下文打包 + 请求体流式上传 + 构建进度列表 + 失败 step 定位）——**已交付 0.8.0**，见上方 DoD 自查 |
